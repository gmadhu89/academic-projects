...
class Data():
    ...
    def dataAllocation(self,path):
        ...
        x_data = file_data[['x1','x2','x3','x4','x5','x6','x7','x8']]
        y_data = file_data[['y']].squeeze()
...
class LinearRegressionModel():
    ...
    def linearClassifier(self,x_train, x_test, y_train):
        ...
        y_predict_train = linear_classifier.predict(x_train)
        y_predict_test = linear_classifier.predict(x_test)
    ...
    def lgTrainAccuracy(self,y_train,y_predict_train):
        ...
        # return: float
        # -------------------------------
        y_predict_train_rounded = np.where(y_predict_train >= 0.5,1,0)
        train_accuracy = accuracy_score(y_train.to_numpy(), y_predict_train_rounded, normalize=True)
    ...
    def lgTestAccuracy(self,y_test,y_predict_test):
        ...
        # return: float
        # -------------------------------
        y_predict_test_rounded = np.where(y_predict_test >= 0.5,1,0)
        test_accuracy = accuracy_score(y_test.to_numpy(), y_predict_test_rounded, normalize=True)
...
class RFClassifier():
    ...
    def rfTrainAccuracy(self,y_train,y_predict_train):
        ...
        # return: float
        # -------------------------------
        train_accuracy = accuracy_score(y_train.to_numpy(), y_predict_train, normalize=True)
    ...
    def rfTestAccuracy(self,y_test,y_predict_test):
        ...
        # return: float
        # -------------------------------
        test_accuracy = accuracy_score(y_test.to_numpy(), y_predict_test, normalize=True)
...
# Q3.3.2 Hyper-parameter Tuning
    ...
    def hyperParameterTuning(self,rf_clf,x_train,y_train):
        ...
        # 'max_depth': [2, 8, 16]
        # -------------------------------
        parameters = {'max_depth':[2,8,16],'n_estimators':[4, 16, 256]}
        gscv_rfc = GridSearchCV(estimator = rf_clf, param_grid = parameters)
...
# Q3.4.1 Pre-process
    ...
    def dataPreProcess(self,x_train,x_test):
        ...
        scaled_x_train = pd.DataFrame(trans_x_train, index=x_train.index , columns=x_train.columns)


        trans_x_test = scaler.transform(x_test)
        scaled_x_test = pd.DataFrame(trans_x_test, index=x_test.index , columns=x_test.columns)
...
# Q3.4.3 Hyper-parameter Tuning
    ...
    def SVMBestScore(self, scaled_x_train, y_train):
        ...
        svc_clf = SVC(gamma='auto').fit(scaled_x_train,y_train)
        svm_cv = GridSearchCV(estimator = svc_clf, param_grid = svm_parameters, n_jobs = -1, return_train_score=True)
